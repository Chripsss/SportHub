...
uint8_t broadcastAddress[5][6] = {
  {0x5C, 0x01, 0x3B, 0x33, 0xB7, 0xF8},
  {0xCC, 0xDB, 0xA7, 0x2D, 0xEF, 0xC0},
  {0xA4, 0xCF, 0x12, 0x44, 0x03, 0x9C},
  {0xF4, 0x65, 0x0B, 0x55, 0x41, 0x3C},
  {0xF4, 0x65, 0x0B, 0x5A, 0x90, 0x14}
};
...
void poleWait() {
  for (int i = 0; i < availablePole; i++) {
    esp_err_t result = esp_now_send(broadcastAddress[i], (uint8_t *) &commandWait, sizeof(commandWait));
  }
}

void poleCount() {
  for (int i = 0; i < availablePole; i++) {
    esp_err_t result = esp_now_send(broadcastAddress[i], (uint8_t *) &commandCount, sizeof(commandCount));
  }
}
