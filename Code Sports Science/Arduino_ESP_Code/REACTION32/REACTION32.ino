...
void loop() {
  if (Serial.available()) {
    ...
    float totalPoint = data.substring(0, data.indexOf(',')).toFloat();
    data = data.substring(data.indexOf(',') + 1);
    ...
    String link = "http://192.168.0.199:80/database/REACTION_TRAINING32.php?a=" + String(uid) + "&b=" + String(right) + "&c=" + String(fastest, 3) + "&d=" + String(slowest, 3) + "&e=" + String(avg, 3) + "&f=" + String(accuracy, 2)+ "&g=" + String(totalPoint);
    HTTPClient http;
