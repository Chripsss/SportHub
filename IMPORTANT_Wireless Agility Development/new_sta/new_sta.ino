...
void loop() {
  ...
  while (boardsStruct[0].pass == 0) delay(10);
...
void debug() {
  if (!debugRun) {
    ...
    for (int i = 0; i < maxPole; i++) {
      if (boardsStruct[i].pass == 1) availablePole++;
    }
    debugRun = true;
  }
...
void gameStart(int path) {
  ...
  for (int i = 1; i < availablePole; i++) {
    ...
    while (boardsStruct[i].pass == 0) delay(10);
    ...
    if (!correct) {
      wrong++;
      while (boardsStruct[i].pass == value) delay(10);
    }

    sendCommand(poleIPs[i - 1], commandOn);
    if (i == availablePole - 1) sendCommand(poleIPs[i], commandOn);
    delay(100);
  }
}
