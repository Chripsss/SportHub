...
void loop() {
  while (state_RFID == 0) {
    ...
    if (digitalRead(buttonUp) == 0) {  //to enter debug mode, hold button for 3 seconds
      timeout = millis();
      while (digitalRead(buttonUp) == 0 && debugMode == 0) {
...
void debug() {
  ...
  while (debugMode == 1) {
    ...
    if (digitalRead(buttonUp) == 0) {  //to quit debug mode, hold Button for 3 seconds
      timeout = millis();
      num = distance[n - 10];
      num++;
      distance[n - 10] = num % 10;
      while (digitalRead(buttonUp) == 0 && debugMode == 1) {
        if ((millis() - timeout) >= 2500) {
          debugMode = 0;
          digitalWrite(buzzerPin, HIGH);
          delay(500);
