...
void playGame() {
  ...
  while (analogRead(sensor1) < param1 && analogRead(sensor2) < param2 && analogRead(sensor3) < param3 && analogRead(sensor4) < param4) {
    //waiting to pressed
  }

  if (analogRead(buttonPressed) > param && state == 1) {
...
//bool sensorState( int buttonPressed)

void sensorReading(int buttonPressed) {
  if (buttonPressed = 54) {
    int samples = 10;
    for (int i = 0; i < samples; i++) {
    ...
    sensorRead /= samples;
  } else if (buttonPressed = 55) {
    int samples = 10;
    for (int i = 0; i < samples; i++) {
      sensorRead += analogRead(sensor2);
    }
    sensorRead /= samples;
  } else if (buttonPressed = 56) {
    int samples = 10;
    for (int i = 0; i < samples; i++) {
      sensorRead += analogRead(sensor3);
    }
    sensorRead /= samples;
  } else if (buttonPressed = 57) {
    int samples = 10;
    for (int i = 0; i < samples; i++) {
      sensorRead += analogRead(sensor4);
    }
    sensorRead /= samples;
  }
}
...
void boomerangBlue(int pixelDelay) {
}
